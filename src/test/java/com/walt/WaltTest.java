...
public class WaltTest {
    ...
    public void testSuccessfulDelivery() {
        ...
        Date deliveryTime = Date.from(Instant.now());
    ...
    @Test
    public void testDifferentCities() {
        Customer customer = customerRepository.findByName("Bach");
        ...
        try {
            ...
            assertEquals(e.getMessage(), WaltServiceImpl.ERR_MISMATCH_CITIES);
            return;
        }
    ...
    public void testNoDriversAvailable() {
        ...
        try {
            ...
            assertEquals(e.getMessage(), WaltServiceImpl.ERR_NO_AVAILABLE_DRIVERS);
            return;
        }
    ...
    @Test
    public void testOneDriverTwoOrders() {
        driverRepository.deleteAll();
        Driver mary = new Driver("Mary", cityRepository.findByName("Tel-Aviv"));
    ...
    @Test
    public void testDriverDistanceSelection() {
        driverRepository.deleteAll();
        ...
        Date deliveryTime = new Date(1000000000);


        Delivery firstDelivery = waltService.createOrderAndAssignDriver(customer, restaurant, deliveryTime);
        Delivery secondDelivery = waltService.createOrderAndAssignDriver(customer, restaurant, deliveryTime);

        Date otherDeliveryTime = Date.from(Instant.now());
        waltService.createOrderAndAssignDriver(customer, restaurant, otherDeliveryTime);

        Driver expectedTwDeliveriesDriver = firstDelivery.getDistance() < secondDelivery.getDistance() ? firstDelivery.getDriver() : secondDelivery.getDriver();
        List<Delivery> deliveryList = deliveryRepository.findAllDeliveriesByDriver(expectedTwDeliveriesDriver);

        assertEquals(deliveryList.size(), 2);
    }
}
